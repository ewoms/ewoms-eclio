...
BOOST_AUTO_TEST_CASE(RestartCWD) {
    ...
    Ewoms::Parser parser;
    {
        Ewoms::Deck deck = parser.parseFile("simulation/CASE.DATA");
        Ewoms::InitConfig init_config(deck);
        BOOST_CHECK_EQUAL(init_config.getRestartRootName(), "simulation/BASE");
    }
    {
      Ewoms::Deck deck = parser.parseFile("simulation/CASE5.DATA");
      Ewoms::InitConfig init_config(deck);
      BOOST_CHECK_EQUAL(init_config.getRestartRootName(), "/abs/path/BASE");
    }
    {
        Ewoms::Deck deck = parser.parseFile("CWD_CASE.DATA");
        Ewoms::InitConfig init_config(deck);
        BOOST_CHECK_EQUAL(init_config.getRestartRootName(), "BASE");
    }
    {
      Ewoms::Deck deck = parser.parseFile("CASE5.DATA");
      Ewoms::InitConfig init_config(deck);
      BOOST_CHECK_EQUAL(init_config.getRestartRootName(), "/abs/path/BASE");
    }
}
