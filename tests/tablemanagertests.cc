...
Ewoms::Deck createSingleRecordDeckWithFullJFunc() {
    const char *deckData =
        "RUNSPEC\nENDSCALE\n2* 1 2 /\nPROPS\n"
...
Ewoms::Deck createSingleRecordDeckWithJFuncBrokenFlag() {
    const char *deckData =
        "RUNSPEC\nENDSCALE\n2* 1 2 /\nPROPS\n"
        "JFUNC\n GARBAGE 55.0 88.0 /\n"
        "TABDIMS\n 2 /\n";
...
Ewoms::Deck createSingleRecordDeckWithJFuncBrokenDirection() {
    const char *deckData =
        "RUNSPEC\nENDSCALE\n2* 1 2 /\nPROPS\n"
        "JFUNC\n * * * * * XZ /\n"
        "TABDIMS\n 2 /\n";
...
BOOST_AUTO_TEST_CASE( CreateTablesWithJFunc ) {
    ...
    const auto& tt = swfnTab.getTable<Ewoms::SwfnTable>(0);
...
BOOST_AUTO_TEST_CASE(PbvdTable_Tests) {
    ...
    BOOST_CHECK_THROW(Ewoms::PbvdTable pbvdTable2(deck.getKeyword("PBVD").getRecord(1).getItem(0)), std::invalid_argument);
}
...
BOOST_AUTO_TEST_CASE(PdvdTable_Tests) {
    ...
    // depth must be increasing down the column.
    BOOST_CHECK_THROW(Ewoms::PdvdTable pdvdTable2(deck.getKeyword("PDVD").getRecord(1).getItem(0)), std::invalid_argument);
}
...
BOOST_AUTO_TEST_CASE(PlyadsTable_Tests) {
    {
        ...
        Ewoms::PlyadsTable plyadsTable(plyadsKeyword.getRecord(0).getItem(0));
...

    {
        ...
        BOOST_CHECK_THROW(Ewoms::PlyadsTable(plyadsKeyword.getRecord(0).getItem(0)), std::invalid_argument);
    }
...
BOOST_AUTO_TEST_CASE(FoamadsTable_Tests) {
    {
        ...
        Ewoms::FoamadsTable foamadsTable(foamadsKeyword.getRecord(0).getItem(0));
...

    {
        ...
        BOOST_CHECK_THROW(Ewoms::FoamadsTable(foamadsKeyword.getRecord(0).getItem(0)), std::invalid_argument);
    }
...
BOOST_AUTO_TEST_CASE(FoammobTable_Tests) {
    {
        ...
        Ewoms::FoammobTable foammobTable(foammobKeyword.getRecord(0).getItem(0));
...

    {
        ...
        BOOST_CHECK_THROW(Ewoms::FoammobTable(foammobKeyword.getRecord(0).getItem(0)), std::invalid_argument);
    }
...
2 2 2 2 46.5 47.5 48.5 / \n";
    ...
    BOOST_CHECK_EQUAL(vfpprodTable.getFloType(), Ewoms::VFPProdTable::FLO_LIQ);
    BOOST_CHECK_EQUAL(vfpprodTable.getWFRType(), Ewoms::VFPProdTable::WFR_WCT);
    BOOST_CHECK_EQUAL(vfpprodTable.getGFRType(), Ewoms::VFPProdTable::GFR_GOR);
    BOOST_CHECK_EQUAL(vfpprodTable.getALQType(), Ewoms::VFPProdTable::ALQ_UNDEF);
...
BOOST_AUTO_TEST_CASE(JFuncTestThrowingOnBrokenData) {
    ...
    BOOST_CHECK_THROW(Ewoms::TableManager tm (deck), std::invalid_argument);

    auto deck2 = createSingleRecordDeckWithJFuncBrokenDirection();
    BOOST_CHECK_THROW(Ewoms::TableManager tm2 (deck2), std::invalid_argument);
}
...
BOOST_AUTO_TEST_CASE(JFuncTest) {
    ...
    const Ewoms::JFunc& jt = tables.getJFunc();
    BOOST_CHECK(jt.flag() == Ewoms::JFunc::Flag::BOTH);
    ...
    BOOST_CHECK(jt2.direction() == Ewoms::JFunc::Direction::Z); // default
}
...
BOOST_AUTO_TEST_CASE( TestPLYMWINJ ) {
    ...
    Ewoms::Parser parser;
    const Ewoms::Deck deck = parser.parseString(inputstring);
...
/
        ...
        const auto tabMgr = ::Ewoms::TableManager {
