...
namespace {
    ...
    namespace SPE1 {            // Family II (SGFN, SOF{2,3}, SWFN)
        namespace TwoPhase {
            namespace GasOil {
                Ewoms::EclipseState satfuncTables()
                {
                    const auto* input = R"(
RUNSPEC
OIL
...
BOOST_AUTO_TEST_CASE (Oil_Water_Family_One)
{
    ...
    auto tables = Ewoms::Tables{ es.getUnits() };
...
namespace {
    Ewoms::EclipseState parse(const std::string& rspec,
                            const std::string& props)
    {
        return {
            Ewoms::Parser{}.parseString(rspec + R"(GRID
INIT
...
{
    // This checks that EFlow's extended table allocation scheme does what
    ...
    // NTPVT, EFlow does not really care about these declared maximum sizes.
...
END
    ...
    const auto es = ::Ewoms::EclipseState { Ewoms::Parser{}.parseString(input) };
