...
Ewoms::EclIO::ERsm create(const std::string& rsm_data) {
    WorkArea work_area("test_ERsm");
    {
        std::ofstream os("TEST.RSM");
        os << rsm_data;
    }
    return Ewoms::EclIO::ERsm("TEST.RSM");
}
...
BOOST_AUTO_TEST_CASE(ERsm) {
    BOOST_CHECK_THROW(Ewoms::EclIO::ERsm("file/does/not/exist"), std::invalid_argument);
    ...
    std::vector<Ewoms::TimeStampUTC> expected_dates = {{1999, 3, 1},
                                                     {1999, 3, 3},
                                                     {1999, 3, 4},
                                                     {1999, 3, 5}};
