...
namespace {
    int ecl_file_get_num_named_kw(Ewoms::EclIO::ERst&  rst,
                                  const std::string& kw)
    {
        int count = 0;
    ...
    ecl_file_iget_named_kw(Ewoms::EclIO::ERst&  rst,
                           const std::string& kw,
                           const int          seqnum)
    {
        ...
        return EclIO::EclFile::EclEntry{ "NoSuchKeyword", Ewoms::EclIO::eclArrType::MESS, 0 };
    }
        ...
        auto sumState = sim_state();
        {
...

            {
                ...
                auto rstFile = OS::Restart {
                    OS::ResultSet{ outputDir, "EWOMS_FILE" }, seqnum,
...

            {
                const auto rstFile = ::Ewoms::EclIO::OutputStream::
                    outputFileName({outputDir, "EWOMS_FILE"}, "UNRST");
...

            {
                ...
                BOOST_CHECK_MESSAGE(!rst.hasKey("OPM_IWEL"), "Restart file must NOT have EWOMS_IWEL vector");
                BOOST_CHECK_MESSAGE(!rst.hasKey("OPM_XWEL"), "Restart file must NOT have EWOMS_XWEL vector");
            }
        }
    }
}
