...
namespace {
    ...
    /*
     Ewoms::UDQActive udq_active() {
      int update_count = 0;
      // construct record data for udq_active
      Ewoms::UDQParams params;
      Ewoms::UDQConfig conf(params);
      Ewoms::UDQActive udq_act;
      Ewoms::UDAValue uda1("WUOPRL");
      update_count += udq_act.update(conf, uda1, "PROD1", Ewoms::UDAControl::WCONPROD_ORAT);

      Ewoms::UDAValue uda2("WULPRL");
      update_count += udq_act.update(conf, uda2, "PROD1", Ewoms::UDAControl::WCONPROD_LRAT);
      Ewoms::UDAValue uda3("WUOPRU");
      update_count += udq_act.update(conf, uda3, "PROD2", Ewoms::UDAControl::WCONPROD_ORAT);
      Ewoms::UDAValue uda4("WULPRU");
      update_count += udq_act.update(conf, uda4, "PROD2", Ewoms::UDAControl::WCONPROD_LRAT);
