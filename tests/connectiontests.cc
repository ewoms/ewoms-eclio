...
Ewoms::WellConnections loadCOMPDAT(const std::string& compdat_keyword) {
...
BOOST_AUTO_TEST_CASE(ActiveCompletions) {
    ...
    Ewoms::WellConnections active_completions(completions, grid);
...
BOOST_AUTO_TEST_CASE(loadCOMPDATTEST) {
    Ewoms::UnitSystem units(Ewoms::UnitSystem::UnitType::UNIT_TYPE_METRIC); // Unit system used in deck FIRST_SIM.DATA.
    {
        const std::string deck = R"(COMPDAT
--                                    CF      Diam    Kh      Skin   Df
    'WELL'  1  1   1   1 'OPEN' 1*    1.168   0.311   107.872 1*     1*  'Z'  21.925 /
/)";
        Ewoms::WellConnections connections = loadCOMPDAT(deck);
...
/)";
        ...
        const auto& conn0 = connections[0];
        BOOST_CHECK_EQUAL(conn0.CF(), units.to_si(Ewoms::UnitSystem::measure::transmissibility, 1.168));
        BOOST_CHECK_EQUAL(conn0.Kh(), units.to_si(Ewoms::UnitSystem::measure::effective_Kh, 0.10 * 1.0));
    }
}
...
BOOST_AUTO_TEST_CASE(loadCOMPDATTESTSPE1) {
    ...
    Ewoms::Schedule sched(deck, state);
...
END
    ...
    const auto actCells = Ewoms::ActiveGridCells {
...
END
...

    {
        ...
        BOOST_CHECK_MESSAGE(  static_cast<bool>(complnum_100), "Completion number must be defined at time 0 for connection in cell (10,10,1)");
        BOOST_CHECK_MESSAGE(! static_cast<bool>(complnum_200), "Completion number must NOT be defined at time 0 for connection in cell (10,10,2)");
...

    {
        ...
        BOOST_CHECK_MESSAGE(static_cast<bool>(complnum_100), "Completion number must be defined at time 0 for connection in cell (10,10,1)");
        BOOST_CHECK_MESSAGE(static_cast<bool>(complnum_200), "Completion number must be defined at time 0 for connection in cell (10,10,2)");
