...
BOOST_AUTO_TEST_CASE( handle_empty_title ) {
    ...
    BOOST_CHECK_EQUAL( "eWoms", deck.getKeyword( "TITLE" ).getStringData().front() );
...
std::unique_ptr<Ewoms::ParserKeyword> ConstructParserKeywordFromJson_withAlternatingRecords()
{
    ...
    Json::JsonObject jsonObject( json_string );
    return std::make_unique<ParserKeyword>( jsonObject );
}

BOOST_AUTO_TEST_CASE(ConstructFromJson_withAlternatingRecords) {
    const std::string json_string = R"(
    {"name" : "FOOSTOG", "sections" : ["PROPS"] , "num_tables" : {"keyword" : "TABDIMS" , "item" : "NTPVT"}, "alternating_records" : [[
      {"name" : "ref_oil_pressure", "value_type" : "DOUBLE"}], [
      {"name" : "oil_phase_pressure" , "value_type" : "DOUBLE"},
      {"name" : "surface_rension", "value_type" : "DOUBLE"}]]}
    )";
...
BOOST_AUTO_TEST_CASE(GetAlternatingKeywordFromParser) {
    ...
    BOOST_CHECK_EQUAL(record0.get(0).name(), "ref_oil_pressure");
    BOOST_CHECK_EQUAL(record1.get(0).name(), "oil_phase_pressure");
...
FIELD
    ...
    /*
      The ewoms-eclio will fail hard if you change unit system with one of the
