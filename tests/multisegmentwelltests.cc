...
/
    ...
    const auto aicd_map = Ewoms::AutoICD::fromWSEGAICD(wsegaicd);
    ...
    const Ewoms::AutoICD& aicd0 = aicd_vector[0].second;
    ...
    BOOST_CHECK(Ewoms::Segment::SegmentType::AICD == segment.segmentType());

    auto aicd = segment.autoICD();
    BOOST_CHECK_GT(aicd.maxAbsoluteRate(), 1.e99);
    BOOST_CHECK(aicd.status()==Ewoms::ICDStatus::SHUT);
...
/
    ...
    const Ewoms::SICD& sicd0 = sicd_vector[0].second;
    ...
    BOOST_CHECK(sicd.status()==Ewoms::ICDStatus::SHUT);
    // 0.002 bars*day*day/Volume^2
