...
BOOST_AUTO_TEST_CASE(CreateMissingDIMENS_throws) {
    Ewoms::Deck deck;
    Ewoms::Parser parser;
    deck.addKeyword( Ewoms::DeckKeyword( parser.getKeyword("RUNSPEC" )));
    deck.addKeyword( Ewoms::DeckKeyword( parser.getKeyword("GRID" )));
    deck.addKeyword( Ewoms::DeckKeyword( parser.getKeyword("EDIT" )));

    BOOST_CHECK_THROW(Ewoms::EclipseGrid{ deck } , std::invalid_argument);
}

static Ewoms::Deck createDeckHeaders() {
    const char* deckData =
        "RUNSPEC\n"
        "\n"
        "DIMENS\n"
...
static Ewoms::Deck createDeckDIMENS() {
    const char* deckData =
        "RUNSPEC\n"
        "\n"
        "DIMENS\n"
...
static Ewoms::Deck createDeckSPECGRID() {
    const char* deckData =
        "GRID\n"
        "SPECGRID \n"
        "  13 17 19 / \n"
...
static Ewoms::Deck createDeckMissingDIMS() {
    const char* deckData =
        "RUNSPEC\n"
        "\n"
        "GRID\n"
...
BOOST_AUTO_TEST_CASE(MissingDimsThrows) {
    Ewoms::Deck deck = createDeckMissingDIMS();
    BOOST_CHECK_THROW( Ewoms::EclipseGrid{ deck }, std::invalid_argument);
}

BOOST_AUTO_TEST_CASE(HasGridKeywords) {
    Ewoms::Deck deck = createDeckHeaders();
    BOOST_CHECK( !Ewoms::EclipseGrid::hasCornerPointKeywords( deck ));
    BOOST_CHECK( !Ewoms::EclipseGrid::hasCartesianKeywords( deck ));
}
...
BOOST_AUTO_TEST_CASE(CheckGridIndex) {
    Ewoms::EclipseGrid grid(17, 19, 41); // prime time
...
static Ewoms::Deck createPinchedCPDeck() {
    const char* deckData =
        "RUNSPEC\n"
        "\n"
        "DIMENS\n"
...
static Ewoms::Deck createMinpvDefaultCPDeck() {
    const char* deckData =
        "RUNSPEC\n"
        "\n"
        "DIMENS\n"
...
static Ewoms::Deck createMinpvCPDeck() {
    const char* deckData =
        "RUNSPEC\n"
        "\n"
        "DIMENS\n"
...
static Ewoms::Deck createCARTDeckDEPTHZ() {
    const char* deckData =
        "RUNSPEC\n"
        "\n"
        "DIMENS\n"
...
static Ewoms::Deck createCARTInvalidDeck() {
    const char* deckData =
        "RUNSPEC\n"
        "\n"
        "DIMENS\n"
...
BOOST_AUTO_TEST_CASE(DEPTHZ_EQUAL_TOPS) {
    Ewoms::Deck deck1 = createCARTDeck();
    Ewoms::Deck deck2 = createCARTDeckDEPTHZ();
...
BOOST_AUTO_TEST_CASE(HasCartKeywordsDEPTHZ) {
    Ewoms::Deck deck = createCARTDeckDEPTHZ();
    BOOST_CHECK( !Ewoms::EclipseGrid::hasCornerPointKeywords( deck ));
    BOOST_CHECK(  Ewoms::EclipseGrid::hasCartesianKeywords( deck ));
}

BOOST_AUTO_TEST_CASE(HasINVALIDCartKeywords) {
    Ewoms::Deck deck = createCARTInvalidDeck();
    BOOST_CHECK( !Ewoms::EclipseGrid::hasCornerPointKeywords( deck ));
    BOOST_CHECK( !Ewoms::EclipseGrid::hasCartesianKeywords( deck ));
}
...
static Ewoms::Deck createInvalidDXYZCARTDeck() {
    const char* deckData =
        "RUNSPEC\n"
        "\n"
        "DIMENS\n"
...
static Ewoms::Deck createInvalidDXYZCARTDeckDEPTHZ() {
    const char* deckData =
        "RUNSPEC\n"
        "\n"
        "DIMENS\n"
...
static Ewoms::Deck createOnlyTopDZCartGrid() {
    const char* deckData =
        "RUNSPEC\n"
        "\n"
        "DIMENS\n"
...
static Ewoms::Deck createInvalidDEPTHZDeck1 () {
    const char* deckData =
        "RUNSPEC\n"
        "\n"
        "DIMENS\n"
...
static Ewoms::Deck createInvalidDEPTHZDeck2 () {
    const char* deckData =
        "RUNSPEC\n"
        "\n"
        "DIMENS\n"
...
/// have ACTNUM = 0
static Ewoms::Deck createActnumBoxDeck() {
    const char* deckData = "RUNSPEC\n"
            "\n"
            "DIMENS \n"
            "  10 10 10 / \n"
...
BOOST_AUTO_TEST_CASE(GridActnumVia3D) {
    ...
    const auto& fp = es.fieldProps();
    const auto& grid = es.getInputGrid();
    Ewoms::EclipseGrid grid2( grid );

    std::vector<int> actnum = {1, 1, 0, 1, 1, 0, 1, 1};
    Ewoms::EclipseGrid grid3( grid , actnum);
...
BOOST_AUTO_TEST_CASE(ProcessedCopy) {
    ...
    Ewoms::EclipseGrid gd1(gd , actnum );
    BOOST_CHECK( gd.equal( gd1 ));
    {
        Ewoms::EclipseGrid gd2(gd , zcorn.data() , actnum );
...
BOOST_AUTO_TEST_CASE(regularCartGrid) {
    ...
    Ewoms::EclipseGrid grid(nx, ny, nz, dx, dy, dz);
...
BOOST_AUTO_TEST_CASE(ZcornMapper) {
    ...
    Ewoms::ZcornMapper zmp = grid.zcornMapper( );
...
BOOST_AUTO_TEST_CASE(MoveTest) {
    ...
    int ny = 4;
    int nz = 5;
    Ewoms::EclipseGrid grid1(nx,ny,nz);
    Ewoms::EclipseGrid grid2( std::move( grid1 )); // grid2 should be move constructed from grid1

    BOOST_CHECK( !grid1.circle( ));
}

static Ewoms::Deck radial_missing_INRAD() {
    const char* deckData =
        "RUNSPEC\n"
        "\n"
        "DIMENS\n"
...
static Ewoms::Deck radial_keywords_DZV_size_mismatch() {
    const char* deckData =
        "RUNSPEC\n"
        "\n"
        "DIMENS\n"
...
static Ewoms::Deck radial_keywords_DZ_size_mismatch() {
    const char* deckData =
        "RUNSPEC\n"
        "\n"
        "DIMENS\n"
...
static Ewoms::Deck radial_keywords_DTHETAV_size_mismatch() {
    const char* deckData =
        "RUNSPEC\n"
        "\n"
        "DIMENS\n"
...
static Ewoms::Deck radial_keywords_TOPS_size_mismatch() {
    const char* deckData =
        "RUNSPEC\n"
        "\n"
        "DIMENS\n"
...
static Ewoms::Deck radial_keywords_ANGLE_OVERFLOW() {
    const char* deckData =
        "RUNSPEC\n"
        "\n"
        "DIMENS\n"
...
BOOST_AUTO_TEST_CASE(RadialKeywords_SIZE_ERROR) {
    BOOST_CHECK_THROW( Ewoms::EclipseGrid{ radial_keywords_DRV_size_mismatch() } , std::invalid_argument);
    BOOST_CHECK_THROW( Ewoms::EclipseGrid{ radial_keywords_DZV_size_mismatch() } , std::invalid_argument);
    BOOST_CHECK_THROW( Ewoms::EclipseGrid{ radial_keywords_DZ_size_mismatch() } , std::invalid_argument);
    BOOST_CHECK_THROW( Ewoms::EclipseGrid{ radial_keywords_TOPS_size_mismatch() } , std::invalid_argument);
    BOOST_CHECK_THROW( Ewoms::EclipseGrid{ radial_keywords_DTHETAV_size_mismatch() } , std::invalid_argument);
...
static Ewoms::Deck spider_details_dz() {
    const char* deckData =
        "RUNSPEC\n"
        "\n"
        "DIMENS\n"
...
BOOST_AUTO_TEST_CASE(SpiderDetailsDZ) {
    Ewoms::Deck deck = spider_details_dz();
    Ewoms::EclipseGrid grid( deck );
...
static Ewoms::Deck radial_details_dz() {
    const char* deckData =
        "RUNSPEC\n"
        "\n"
        "DIMENS\n"
...
BOOST_AUTO_TEST_CASE(RadialDetailsDZ) {
    Ewoms::Deck deck = radial_details_dz();
    Ewoms::EclipseGrid grid( deck );
...
BOOST_AUTO_TEST_CASE(CoordMapper) {
    size_t nx = 10;
    size_t ny = 7;
    Ewoms::CoordMapper cmp = Ewoms::CoordMapper( nx , ny );
...
static Ewoms::Deck createCARTDeckTest3x4x2() {
    const char* deckData =
        "RUNSPEC\n"
        "\n"
        "DIMENS\n"
...
BOOST_AUTO_TEST_CASE(CART_Deck_3x4x2) {
    ...
    Ewoms::Deck deck1 = createCARTDeckTest3x4x2();
    Ewoms::EclipseGrid grid1( deck1 );
...
static Ewoms::Deck createCARTDeckDEPTHZ_2x3x2() {
    const char* deckData =
        "RUNSPEC\n"
        "\n"
        "DIMENS\n"
...
BOOST_AUTO_TEST_CASE(CART_Deck_DEPTHZ_2x3x2) {
    ...
    Ewoms::Deck deck1 = createCARTDeckDEPTHZ_2x3x2();
    Ewoms::EclipseGrid grid1( deck1 );
...
static Ewoms::Deck BAD_CP_GRID_MAPAXES() {
    const char* deckData =
        "RUNSPEC\n"
        "\n"
        "DIMENS\n"
...
BOOST_AUTO_TEST_CASE(SAVE_FIELD_UNITS) {
    ...
    Ewoms::EclipseState es(deck);
    Ewoms::UnitSystem units = es.getDeckUnitSystem();
...
BOOST_AUTO_TEST_CASE(SAVE_METRIC_UNITS) {
    ...
    Ewoms::UnitSystem units1 = es1.getDeckUnitSystem();
    const auto length = ::Ewoms::UnitSystem::measure::length;
    ...
    Ewoms::EclipseState es2(deck2);
    Ewoms::UnitSystem units2 = es2.getDeckUnitSystem();
...
BOOST_AUTO_TEST_CASE(ExportMAPAXES_TEST) {
    ...
    Ewoms::Deck deck1 = BAD_CP_GRID_MAPAXES();
    Ewoms::EclipseGrid grid1( deck1 );
    ...
    Ewoms::EclipseGrid grid3(dims, coord, zcorn, actnum.data(), mapaxes.data());

    BOOST_CHECK( grid3.equal( grid1 ));

    mapaxes[1] = 101;
    Ewoms::EclipseGrid grid4(dims, coord, zcorn, actnum.data(), mapaxes.data());
...
BOOST_AUTO_TEST_CASE(TESTCP_ACTNUM_UPDATE) {
    ...
    Ewoms::EclipseGrid grid1( deck);
    Ewoms::EclipseGrid grid2( deck, newAct.data());
...
BOOST_AUTO_TEST_CASE(TEST_altGridConstructors) {
    ...
    Ewoms::EclipseGrid grid2( grid1 , zcorn.data(), actnum);
    //Ewoms::EclipseGrid grid2( grid1 , zcorn, actnum);

    BOOST_CHECK( grid1.equal( grid2) );

    std::vector<double> emptyZcorn;

    Ewoms::EclipseGrid grid3( grid1 , emptyZcorn.data(), actnum);
    BOOST_CHECK( grid1.equal( grid3) );
}

static Ewoms::Deck BAD_CP_GRID_ACTNUM() {
    const char* deckData =
        "RUNSPEC\n"
        "\n"
        "DIMENS\n"
...
BOOST_AUTO_TEST_CASE(TEST_getCellCenters) {
    ...
    Ewoms::Deck deck1 = BAD_CP_GRID_ACTNUM();
    Ewoms::EclipseGrid grid1( deck1 );
...
BOOST_AUTO_TEST_CASE(LoadFromBinary) {
    BOOST_CHECK_THROW(Ewoms::EclipseGrid( "No/does/not/exist" ) , std::invalid_argument);
}
...
BOOST_AUTO_TEST_CASE(TEST_constructFromEgrid) {
    ...
    Ewoms::EclipseGrid grid1( deck);
    Ewoms::EclipseGrid grid2( "SPE1CASE1.EGRID");
...
BOOST_AUTO_TEST_CASE(TEST_GDFILE_1) {
    ...
    Ewoms::Parser parser;
    auto deck1 = parser.parseString( deckData1) ;
    BOOST_CHECK_NO_THROW( Ewoms::EclipseGrid grid1(deck1) );
}
...
BOOST_AUTO_TEST_CASE(TEST_GDFILE_2) {
    ...
    Ewoms::EclipseState es1a( deck1a );
    ...
    Ewoms::EclIO::EclFile file1("BAD_CP_M.EGRID");
    ...
    Ewoms::EclipseGrid grid3a( deck3a);
    ...
    Ewoms::EclipseGrid grid3b( deck3b);
    ...
    Ewoms::EclipseGrid grid3c( deck3c);
...
BOOST_AUTO_TEST_CASE(TEST_COLLAPSED_CELL) {
    Ewoms::EclipseGrid grid(2,2,2,1,1,0);
