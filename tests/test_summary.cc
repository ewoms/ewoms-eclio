...
data::Wells result_wells(const bool w3_injector = true)
{
    ...
    // Segment vectors
    auto segment = ::Ewoms::data::Segment{};
    ...
    */
    ...
    well1.current_control.prod = ::Ewoms::Well::ProducerCMode::THP;
    ...
    well6.current_control.inj = ::Ewoms::Well::InjectorCMode::GRUP;
...
data::GroupAndNetworkValues result_group_nwrk()
{
    ...
    grp_nwrk.nodeData["G_1"].pressure = 33.44*Ewoms::unit::barsa;
    grp_nwrk.nodeData["G_2"].pressure = 23.45*Ewoms::unit::barsa;
    grp_nwrk.nodeData["PLAT-A"].pressure = 21.0*Ewoms::unit::barsa;
...
BOOST_AUTO_TEST_CASE(connection_kewords) {
    ...
    /* Solvent flow rate + or - Note ewoms uses negative values for producers, while CNFR outputs positive
...
BOOST_AUTO_TEST_CASE(READ_WRITE_WELLDATA) {
            ...
            Ewoms::data::Wells wellRates = result_wells();
            ...
            BOOST_CHECK_MESSAGE(curr.prod == ::Ewoms::Well::ProducerCMode::CMODE_UNDEFINED, "W_6 must have an undefined producer control");
            BOOST_CHECK_MESSAGE(curr.inj == ::Ewoms::Well::InjectorCMode::GRUP, "W_6 must be on GRUP control");
}
...
BOOST_AUTO_TEST_CASE(Test_SummaryState) {
    ...
    Ewoms::SummaryState st2(std::chrono::system_clock::now());
...
namespace {
    Ewoms::SummaryState calculateRestartVectors(const setup& config)
    {
        ::Ewoms::out::Summary smry {
...
namespace {
    ...
    std::unordered_map<std::size_t, data::Segment> prod01_seg_results()
    {
        ...
        // EFlow's producer rates are negative (positive fluxes well -> reservoir).
        const auto sign = -1.0;
    ...
    std::vector<data::Connection> inje01_conn_results()
    {
        ...
        // EFlow's injection rates are positive (positive fluxes well -> reservoir).
        const auto sign = +1.0;
...
namespace {
    bool hasSegmentVariable_Prod01(const Ewoms::EclIO::ESmry* ecl_sum,
                                   const char*              vector,
                                   const int                segID)
    {
    ...
    double getSegmentVariable_Prod01(const Ewoms::EclIO::ESmry* ecl_sum,
                                     const int                timeIdx,
                                     const char*              vector,
                                     const int                segID)
    {
