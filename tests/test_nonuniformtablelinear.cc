...
#include <ewoms/eclio/utility/numeric/nonuniformtablelinear.hh>

BOOST_AUTO_TEST_CASE(utility_functions)
{
    // Test isNondecreasing().
    using Ewoms::isNondecreasing;
...
BOOST_AUTO_TEST_CASE(table_operations)
{
    ...
    Ewoms::NonuniformTableLinear<double> t1(xv, yv);
    Ewoms::NonuniformTableLinear<double> t1_copy1(xv, yv);
    Ewoms::NonuniformTableLinear<double> t1_copy2(t1);
