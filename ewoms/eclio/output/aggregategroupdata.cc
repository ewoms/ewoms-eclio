...
void groupProductionControllable(const Ewoms::Schedule& sched, const Ewoms::SummaryState& sumState, const Ewoms::Group& group, const size_t simStep, bool& controllable)
{
...
void groupInjectionControllable(const Ewoms::Schedule& sched, const Ewoms::SummaryState& sumState, const Ewoms::Group& group, const Ewoms::Phase& iPhase, const size_t simStep, bool& controllable)
{
    using wellCtrlMode   = ::Ewoms::RestartIO::Helpers::VectorItems::IWell::Value::WellCtrlMode;
    if (controllable)
        return;
                                     ...
                                     const size_t simStep)
{
    ...
    while (current.name() != "FIELD") {
        ...
        if (prod_cmode != Ewoms::Group::ProductionCMode::FLD)
            return true;

        if (prod_cmode != Ewoms::Group::ProductionCMode::NONE)
            return true;
...
void gconprodCMode(const Ewoms::Group& group,
                   const int nwgmax,
                   IGrpArray& iGrp) {
...
template <class IGrpArray>
void productionGroup(const Ewoms::Schedule&     sched,
                     const Ewoms::Group&        group,
                     const int                nwgmax,
                     const std::size_t        simStep,
                     const Ewoms::SummaryState& sumState,
                     ...
                     IGrpArray&               iGrp)
{
    ...
    Ewoms::Group::ProductionCMode active_cmode = Ewoms::Group::ProductionCMode::NONE;
    {
    ...
    */
    ...
    } else if (deck_cmode == Ewoms::Group::ProductionCMode::NONE) {
        iGrp[nwgmax + 5] = 1;
    }
 CGROUP_DONE:
    ...
    */
    ...
    case Ewoms::Group::ProductionCMode::RESV:
        ...
        break;
    case Ewoms::Group::ProductionCMode::FLD:
        if (cgroup && (prod_guide_rate_def != Ewoms::Group::GuideRateTarget::NO_GUIDE_RATE)) {
...
template <class IGrpArray>
void injectionGroup(const Ewoms::Schedule&     sched,
                    const Ewoms::Group&        group,
                    const int                nwgmax,
                    const std::size_t        simStep,
                    const Ewoms::SummaryState& sumState,
        ...
        if ((group.hasInjectionControl(Ewoms::Phase::WATER)) || (group.getGroupType() == Ewoms::Group::GroupType::NONE)) {
            ...
            if (is_field) {
                ...
                const auto& winj_cmode = (group.hasInjectionControl(Ewoms::Phase::WATER))
                    ? group.injectionControls(Ewoms::Phase::WATER, sumState).cmode
                ...
                int higher_lev_winj_cmode
                    = higherLevelInjCMode_NotNoneFld_SeqIndex(sched, sumState, group, Ewoms::Phase::WATER, simStep);
                ...
                if (!group.injectionGroupControlAvailable(Ewoms::Phase::WATER) && (higher_lev_winj_ctrl <= 0)) {
                    ...
                    if (!((winj_cmode == Ewoms::Group::InjectionCMode::FLD)
                          || (winj_cmode == Ewoms::Group::InjectionCMode::NONE))) {
        ...
        if ((group.hasInjectionControl(Ewoms::Phase::GAS)) || (group.getGroupType() == Ewoms::Group::GroupType::NONE)) {
            if (is_field) {
                ...
                const auto& ginj_cmode = (group.hasInjectionControl(Ewoms::Phase::GAS))
                    ? group.injectionControls(Ewoms::Phase::GAS, sumState).cmode
                    : Ewoms::Group::InjectionCMode::NONE;

                int higher_lev_ginj_ctrl = higherLevelInjControlGroupSeqIndex(sched, sumState, group, "GMCTG", simStep);
                int higher_lev_ginj_cmode
                    = higherLevelInjCMode_NotNoneFld_SeqIndex(sched, sumState, group, Ewoms::Phase::GAS, simStep);
                ...
                if (!group.injectionGroupControlAvailable(Ewoms::Phase::GAS) && (higher_lev_ginj_ctrl <= 0)) {
                    ...
                    if (!((ginj_cmode == Ewoms::Group::InjectionCMode::FLD)
                          || (ginj_cmode == Ewoms::Group::InjectionCMode::NONE))) {
...
template <class IGrpArray>
void storeGroupTree(const Ewoms::Schedule& sched,
                    const Ewoms::Group& group,
                    const int nwgmax,
                    const int ngmaxz,
                    const std::size_t simStep,
...
template <class IGrpArray>
void storeEFlowingWells(const Ewoms::Group&        group,
                       const int                nwgmax,
                       const Ewoms::SummaryState& sumState,
                       IGrpArray&               iGrp) {
    using IGroup = ::Ewoms::RestartIO::Helpers::VectorItems::IGroup::index;
    ...
    iGrp[nwgmax + IGroup::EFlowingWells] = static_cast<int>(g_act_pwells) + static_cast<int>(g_act_iwells);
}
                   ...
                   IGrpArray&               iGrp)
{
    ...
    storeEFlowingWells(group, nwgmax, sumState, iGrp);

    iGrp[nwgmax + 17] = -1;
    iGrp[nwgmax + 22] = -1;

    // Treat al groups which are *not* pure injection groups.
    if (group.getGroupType() != Ewoms::Group::GroupType::INJECTION)
    ...
    if (group.getGroupType() != Ewoms::Group::GroupType::PRODUCTION)
