...
namespace {
    ...
    struct WellWrapper {
        ...
        WellWrapper(const Ewoms::Well& well_arg) :
            well(well_arg)
        { }
...
namespace {
    ...
    struct WellConnection {
        ...
        WellConnection(const Ewoms::Well& well_arg, const Ewoms::Connection& connection_arg) :
            well(well_arg),
...
namespace {
    ...
    struct SegmentConnection {
        ...
        SegmentConnection(const Ewoms::Well& well_arg, const Ewoms::Connection& conn_arg, const Ewoms::Segment& segment_arg) :
            well(well_arg),
            connection(conn_arg),
            segment(segment_arg)
        {}
    ...
    struct WellSegment {
        ...
        WellSegment(const Ewoms::Well& well_arg, const Ewoms::Segment& segment_arg) :
            well(well_arg),
            segment(segment_arg)
        {}
...
void Ewoms::RptIO::workers::write_WELSPECS(std::ostream& os, unsigned, const Ewoms::Schedule& schedule, const Ewoms::EclipseGrid& grid, std::size_t report_step) {
            ...
            }
            {
                ...
                msw_connection.print_header(os);
                {
                    ...
                    std::transform(connections.begin(), connections.end(), std::back_inserter(wrapper_data),
                                   [&well, &segments] (const Ewoms::Connection& connection) { return SegmentConnection(well, connection, segments.getFromSegmentNumber(connection.segment())); });
