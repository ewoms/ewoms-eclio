...
namespace {
    ...
    std::string format_number(const Ewoms::UnitSystem& unit_system, Ewoms::UnitSystem::measure measure, double number, std::size_t width) {
    ...
    struct column {
        ...
        Ewoms::optional<Ewoms::UnitSystem::measure> dimension { Ewoms::nullopt } ;
...
namespace {
    ...
    struct WellWrapper {
        ...
        std::string reference_depth(const context& ctx, std::size_t, std::size_t) const {
            return format_number(ctx.unit_system, Ewoms::UnitSystem::measure::length, well.getRefDepth(), 6);
        }
        ...
        */
        ...
        std::string drainage_radius(const context& ctx, std::size_t, std::size_t) const {
            if (well.getDrainageRadius() == 0)
                return "P.EQUIV.R";
            return format_number(ctx.unit_system, Ewoms::UnitSystem::measure::length, well.getDrainageRadius(), 6);
        }

        std::string gas_inflow(const context&, std::size_t, std::size_t) const {
            return Ewoms::Well::GasInflowEquation2String( well.gas_inflow_equation() );
        }
    };
...
namespace {
    ...
    struct WellConnection {
        ...
        std::string centre_depth(const context& ctx, std::size_t, std::size_t) const {
            return format_number(ctx.unit_system, Ewoms::UnitSystem::measure::length, connection.depth(), 6);
        }
        ...
        std::string conn_factor(const context& ctx, std::size_t, std::size_t) const {
            return format_number(ctx.unit_system, Ewoms::UnitSystem::measure::transmissibility, connection.CF(), 10);
        }

        std::string int_diam(const context& ctx, std::size_t, std::size_t) const {
            return format_number(ctx.unit_system, Ewoms::UnitSystem::measure::length, connection.rw() * 2, 8);
        }
...
namespace {
    ...
    struct SegmentConnection {
        ...
        std::string perf_start_length(const context& ctx, std::size_t, std::size_t) const {
            return format_number(ctx.unit_system, Ewoms::UnitSystem::measure::length, perf_range.first, 6);
        }

        std::string perf_mid_length(const context& ctx, std::size_t, std::size_t) const {
            return format_number(ctx.unit_system, Ewoms::UnitSystem::measure::length, (perf_range.first + perf_range.second) / 2.0, 6);
        }

        std::string perf_end_length(const context& ctx, std::size_t, std::size_t) const {
            return format_number(ctx.unit_system, Ewoms::UnitSystem::measure::length, perf_range.second, 6);
        }

        std::string length_end_segmt(const context& ctx, std::size_t, std::size_t) const {
            return format_number(ctx.unit_system, Ewoms::UnitSystem::measure::length, segment.totalLength(), 6);
        }
        ...
        std::string segment_depth(const context& ctx, std::size_t, std::size_t) const {
            return format_number(ctx.unit_system, Ewoms::UnitSystem::measure::length, segment.depth(), 6);
        }

        std::string grid_block_depth(const context& ctx, std::size_t, std::size_t) const {
            return format_number(ctx.unit_system, Ewoms::UnitSystem::measure::length, ctx.grid.getCellDepth( connection.global_index() ), 6);
        }
    ...
    struct WellSegment {
        ...
        std::string well_name_seg(const context&, std::size_t sub_report, std::size_t n) const {
            ...
            if (n == 1)
                return Ewoms::WellSegments::CompPressureDropToString(well.getSegments().compPressureDrop());
        ...
        std::string length(const context& ctx, std::size_t sub_report, std::size_t line_number) const {
            ...
            return format_number(ctx.unit_system, Ewoms::UnitSystem::measure::length, segment.totalLength() - outlet_segment.totalLength(), 6);
        }
        ...
        std::string depth_change(const context& ctx, std::size_t sub_report, std::size_t line_number) const {
            ...
            const auto& segments { well.getSegments() } ;
            const auto& outlet_segment { segments.getFromSegmentNumber( segment.outletSegment() ) } ;
            return format_number(ctx.unit_system, Ewoms::UnitSystem::measure::length, segment.depth() - outlet_segment.depth(), 6);
        }

        std::string internal_diameter(const context& ctx, std::size_t, std::size_t) const {
            const auto number { segment.internalDiameter() } ;

            if (number != Ewoms::Segment::invalidValue()) {
                return format_number(ctx.unit_system, Ewoms::UnitSystem::measure::length, number, 6);
            } else {
                return "0";
            }
        }
        ...
        std::string roughness(const context& ctx, std::size_t, std::size_t) const {
            ...
            if (number != Ewoms::Segment::invalidValue()) {
                return format_number(ctx.unit_system, Ewoms::UnitSystem::measure::length, number, 8);
            } else {
                return "0";
            }
        }
        ...
        std::string volume(const context& ctx, std::size_t, std::size_t) const {
            return format_number(ctx.unit_system, Ewoms::UnitSystem::measure::volume, segment.volume(), 5);
        }
...
void report_well_connection_data(std::ostream& os, const std::vector<Ewoms::Well>& data, const context& ctx) {
    ...
    for (const auto& well : data) {
        ...
        std::transform(connections.begin(), connections.end(), std::back_inserter(wrapper_data), [&well](const Ewoms::Connection& connection) { return WellConnection { well, connection } ; });
...
void Ewoms::RptIO::workers::write_WELSPECS(std::ostream& os, unsigned, const Ewoms::Schedule& schedule, const Ewoms::EclipseGrid& grid, const Ewoms::UnitSystem& unit_system, std::size_t report_step) {
    ...
    for (const auto& well : changed_wells) {
        if (well.isMultiSegment()) {
            {
                const report<Ewoms::Well, WellSegment, 3> msw_data { "MULTI-SEGMENT WELL: SEGMENT STRUCTURE", msw_well_table, ctx};
                ...
                for (const auto& branch : segments.branches()) {
                    ...
                    std::transform(branch_segments.begin(), branch_segments.end(), std::back_inserter(wrapper_data), [&well](const Ewoms::Segment& segment) { return WellSegment { well, segment } ; });
                ...
                msw_data.print_footer(os, {{1, "The pressure drop multiplier is not implemented in eflow and will always show the default value 1.0."}});
            }
            {
                const report<Ewoms::Well, SegmentConnection, 3> msw_connection { "MULTI-SEGMENT WELL: CONNECTION DATA", msw_connection_table,  ctx};
                msw_connection.print_header(os);
                {
                    ...
                    std::transform(connections.begin(), connections.end(), std::back_inserter(wrapper_data),
                                   [&well, &segments] (const Ewoms::Connection& connection) { return SegmentConnection { well, connection, segments.getFromSegmentNumber(connection.segment()), *connection.perf_range() } ; });
