...
void subreport_well_specification_data(std::ostream& os, const std::vector<Ewoms::Well>& data, const context& ctx) {
        subreport<Ewoms::Well, WellWrapper, 3> well_specification { "WELL SPECIFICATION DATA", well_specification_table, ctx};
...
namespace {
    ...
    struct WellSegment {
        ...
        static std::vector<WellSegment> transform(const Ewoms::Well& well) {
            const auto &connections { well.getConnections() } ;
...
void subreport_well_connection_data(std::ostream& os, const std::vector<Ewoms::Well>& data, const context& ctx) {
    const subreport<Ewoms::Well, WellConnection, 3> well_connection { "WELL CONNECTION DATA", connection_table, ctx};
...
void Ewoms::RptIO::workers::write_WELSPECS(std::ostream& os, unsigned, const Ewoms::Schedule& schedule, const Ewoms::EclipseGrid& grid, std::size_t report_step) {
    ...
    for (const auto& well : changed_wells) {
        if (well.isMultiSegment()) {
            const subreport<Ewoms::Well, WellSegment, 3> msw_data { "MULTI-SEGMENT WELL: SEGMENT STRUCTURE", msw_well_table, ctx, '=', &WellSegment::segment_structure_separator};
            const subreport<Ewoms::Well, WellSegment, 3> msw_connection { "MULTI-SEGMENT WELL: CONNECTION DATA", msw_connection_table,  ctx, '='};
