            ...
            }
            {
                ...
                msw_connection.print_header(os);
                {
                    ...
                    std::transform(connections.begin(), connections.end(), std::back_inserter(wrapper_data),
                                   [&well, &segments, &perf_range] (const Ewoms::Connection& connection) { return SegmentConnection { well, connection, segments.getFromSegmentNumber(connection.segment()), perf_range } ; });
