...
#include <ewoms/eclio/parser/eclipsestate/tables/flattable.hh> // pvtw, pvcdo
...
#include <ewoms/eclio/output/vectoritems/tabdims.hh>
...
using Ix = ::Ewoms::RestartIO::Helpers::VectorItems::TabDims::index;
...
namespace {
    ...
    using BuildDep = std::function<
        std::size_t(const std::size_t             tableID,
                    ...
                    ::Ewoms::LinearisedOutputTable& table)
        >;
                        ...
                        const BuildDep&   buildDeps)
    {
        ...
        auto descr = ::Ewoms::DifferentiateOutputTable::Descriptor{};

        auto linTable = ::Ewoms::LinearisedOutputTable {
             ...
             descr.tableID < 1*numTab; ++descr.tableID)
        {
                 ...
                 descr.primID < 1*numPrim; ++descr.primID)
            {
                ...
                //    from namespace ::Ewoms::DifferentiateOutputTable.
...
namespace { namespace SatFunc {
    ...
    namespace Gas {
                 ...
                 const Ewoms::TableContainer& sgfn)
        {
                ...
                -> std::size_t
            {
                ...
                // Pcgo(Sg)
                {
                    const auto uPress = ::Ewoms::UnitSystem::measure::pressure;
    ...
    namespace Oil {
        ...
        namespace ThreePhase {
            ...
            makeSOF3Table(const Ewoms::SgofTable& sgof,
                          const Ewoms::SwofTable& swof)
            {
...
namespace { namespace PVTFunc {
    ...
    namespace Gas {
        ...
        fromPVDG(const std::size_t          numPressNodes,
                 const Ewoms::UnitSystem&     units,
                 const Ewoms::TableContainer& pvdg)
        {
            // Columns [ Pg, 1/Bg, 1/(Bg*mu_g), derivatives ]
            using PVDG = ::Ewoms::PvdgTable;
                ...
                -> std::size_t
            {
                ...
                // Column 1: 1/Bg
                {
                    const auto uRecipFVF = ::Ewoms::UnitSystem::measure::
                ...
                // Column 2: 1/(Bg*mu_g)
                {
                    ...
                    const auto uVisc = ::Ewoms::UnitSystem::measure::viscosity;
        ...
        fromPVTG(const std::size_t                  numCompNodes,
                 ...
                 const Ewoms::UnitSystem&             units,
                 const std::vector<Ewoms::PvtgTable>& pvtg)
        {
                ...
                -> std::size_t
            {
                ...
                // Column 0: Rv
                {
                    const auto uRv = ::Ewoms::UnitSystem::measure::oil_gas_ratio;
        ...
        ///    pvtg.
        std::size_t maxNumCompNodes(const std::vector<Ewoms::PvtgTable>& pvtg)
        {
            ...
            for (const auto& table : pvtg) {
                const auto max_nrpvt_tab =
                    std::accumulate(table.begin(), table.end(), std::size_t{0},
                        [](const std::size_t n, const Ewoms::SimpleTable& t)
                    {
        ...
        ///    pvdo.
        std::size_t maxNumPressNodes(const Ewoms::TableContainer& pvdg)
        {
            using PVDG = ::Ewoms::PvdgTable;
        ...
        ///    pvtg.
        std::size_t maxNumPressNodes(const std::vector<Ewoms::PvtgTable>& pvtg)
        {
            if (pvtg.empty()) { return 0; }

            return std::accumulate(std::begin(pvtg), std::end(pvtg), std::size_t{0},
                [](const std::size_t n, const Ewoms::PvtgTable& t) -> std::size_t
            {
    ...
    namespace Oil {
        ...
        fromPVDO(const std::size_t          numPressNodes,
                 const Ewoms::UnitSystem&     units,
                 const Ewoms::TableContainer& pvdo)
        {
            // Columns [ Po, 1/Bo, 1/(Bo*mu_o), derivatives ]
            using PVDO = ::Ewoms::PvdoTable;
        ...
        fromPVTO(const std::size_t                  numCompNodes,
                 const std::size_t                  numPressNodes,
                 const Ewoms::UnitSystem&             units,
                 const std::vector<Ewoms::PvtoTable>& pvto)
        {
            // Columns [ Po, 1/Bo, 1/(Bo*mu_o), derivatives ]
                ...
                -> std::size_t
            {
                ...
                // Column 0: Po
                {
                    const auto uPo = ::Ewoms::UnitSystem::measure::pressure;
                         ...
                         const std::vector<Ewoms::PvtoTable>& pvto)
        {
            ...
            return createPropfuncTable(numTab, numPrim, numRows, numDep, fillVal,
                ...
                -> std::size_t
            {
                const auto uRs = ::Ewoms::UnitSystem::measure::gas_oil_ratio;
        ...
        /// \return Maximum number of active keys across all tables of \p
        ///    pvto.
        std::size_t maxNumCompNodes(const std::vector<Ewoms::PvtoTable>& pvto)
        {
            if (pvto.empty()) { return 0; }

            return std::accumulate(std::begin(pvto), std::end(pvto), std::size_t{0},
                [](const std::size_t n, const Ewoms::PvtoTable& t) -> std::size_t
            {
                return std::max(n, t.getSaturatedTable().numRows());
            });
        }
        ...
        /// \return Maximum number of table rows across all tables of \p
        ///    pvdo.
        std::size_t maxNumPressNodes(const Ewoms::TableContainer& pvdo)
        {
            using PVDO = ::Ewoms::PvdoTable;
        ...
        std::size_t maxNumPressNodes(const std::vector<Ewoms::PvtoTable>& pvto)
        {
    ...
    namespace Water {
        ...
        std::vector<double>
        fromPVTW(const Ewoms::UnitSystem& units,
                 const Ewoms::PvtwTable&  pvtw)
        {
