...
private:
    using RstFile = std::unique_ptr<Ewoms::EclIO::ERst>;

    using VectorColl = std::unordered_set<std::string>;
    using TypedColl  = std::unordered_map<
        Ewoms::EclIO::eclArrType, VectorColl, std::hash<int>
        >;
...
RestartFileView::RestartFileView(const std::string& filename,
                                 const int          report_step)
    : rst_file_   { new Ewoms::EclIO::ERst{filename} }
...
namespace {
    void throwIfMissingRequired(const Ewoms::RestartKey& rst_key)
    {
        if (rst_key.required) {
    ...
    void insertSolutionVector(const std::vector<double>&           vector,
                              const Ewoms::RestartKey&               value,
                              const std::vector<double>::size_type numcells,
                              Ewoms::data::Solution&                 sol)
    {
        ...
        sol.insert(value.key, value.dim, vector,
                   Ewoms::data::TargetType::RESTART_SOLUTION);
    }

    void loadIfAvailable(const Ewoms::RestartKey&               value,
                         const std::vector<double>::size_type numcells,
    ...
    void loadHysteresisIfAvailable(const std::string&                   primary,
                                   const Ewoms::RestartKey&               fallback_key,
                                   const std::vector<double>::size_type numcells,
                                   RestartFileView&                     rst_view,
                                   Ewoms::data::Solution&                 sol)
    {
        ...
        if (kwdata.empty()) {
            ...
            // Primary exists in rst_view.  Translate to EFlow's hysteresis
            // parameter.
    ...
    void restoreHysteresisVector(const Ewoms::RestartKey& value,
                                 const int              numcells,
                                 RestartFileView&       rst_view,
                                 Ewoms::data::Solution&   sol)
    {
    ...
    Ewoms::data::Solution
    restoreSOLUTION(const std::vector<Ewoms::RestartKey>& solution_keys,
                    const int                           numcells,
                    RestartFileView&                    rst_view)
    {
        Ewoms::data::Solution sol(/* init_si = */ false);

        for (const auto& value : solution_keys) {
            if (isHysteresis(value.key)) {
                // Special case handling of hysteresis data.  Possibly needs
                // translation from ECLIPSE-compatible set to EFlow's known
    ...
    void restoreExtra(const std::vector<Ewoms::RestartKey>& extra_keys,
                      const Ewoms::UnitSystem&              usys,
                      RestartFileView&                    rst_view,
                      Ewoms::RestartValue&                  rst_value)
    {
    ...
    Ewoms::data::Wells
    restore_wells_ewoms(const ::Ewoms::EclipseState& es,
                      ...
                      RestartFileView&           rst_view)
    {
        ...
        std::vector<rt> phases;
        {
            ...
            if (phase.active(Ewoms::Phase::WATER)) { phases.push_back(rt::wat); }
            if (phase.active(Ewoms::Phase::OIL))   { phases.push_back(rt::oil); }
            if (phase.active(Ewoms::Phase::GAS))   { phases.push_back(rt::gas); }
        }
        ...
        for (const auto& sched_well : sched_wells) {
            ...
            for (const auto& sc : sched_well.getConnections()) {
                ...
                if (!grid.cellActive(i, j, k) || sc.state() == Ewoms::Connection::State::SHUT) {
                    opm_xwel_data += Ewoms::data::Connection::restart_size + phases.size();
                    continue;
                }
    ...
    void restoreConnRates(const WellVectors::Window<double>& xcon,
                          ...
                          const bool                         gas,
                          const bool                         wat,
                          Ewoms::data::Connection&             xc)
    {
        using M = ::Ewoms::UnitSystem::measure;

        if (wat) {
            xc.rates.set(Ewoms::data::Rates::opt::wat,
                         - usys.to_si(M::liquid_surface_rate,
                                      xcon[VI::XConn::index::WaterRate]));
        }

        if (oil) {
            xc.rates.set(Ewoms::data::Rates::opt::oil,
                         - usys.to_si(M::liquid_surface_rate,
                                      xcon[VI::XConn::index::OilRate]));
        }

        if (gas) {
            xc.rates.set(Ewoms::data::Rates::opt::gas,
    ...
    void zeroConnRates(const bool             oil,
                       ...
                       Ewoms::data::Connection& xc)
    {
        if (wat) { xc.rates.set(Ewoms::data::Rates::opt::wat, 0.0); }
        if (oil) { xc.rates.set(Ewoms::data::Rates::opt::oil, 0.0); }
        if (gas) { xc.rates.set(Ewoms::data::Rates::opt::gas, 0.0); }
    }

    void restoreConnResults(const Ewoms::Well&       well,
                            ...
                            Ewoms::data::Well&        xw)
    {
        ...
        xw.connections.resize(nConn, Ewoms::data::Connection{});
        ...
        const auto conns = Ewoms::WellConnections(conn0, grid);
                                  ...
                                  Ewoms::data::Well&         xw)
    {
        ...
        // reservoir to well--i.e., towards the producer/platform.  The EFlow
                 ...
                 const SegmentVectors&   segData)
    {
        ...
        if (wat) {
            xw.rates.set(Ewoms::data::Rates::opt::wat,
                         - usys.to_si(M::liquid_surface_rate,
                                      xwel[VI::XWell::index::WatPrRate]));
        }

        if (oil) {
            xw.rates.set(Ewoms::data::Rates::opt::oil,
                         - usys.to_si(M::liquid_surface_rate,
                                      xwel[VI::XWell::index::OilPrRate]));
        }

        if (gas) {
            xw.rates.set(Ewoms::data::Rates::opt::gas,
                         - usys.to_si(M::gas_surface_rate,
    ...
    Ewoms::data::Wells
    restore_wells_ecl(const ::Ewoms::EclipseState&       es,
                      const ::Ewoms::EclipseGrid&        grid,
                      const ::Ewoms::Schedule&           schedule,
                      std::shared_ptr<RestartFileView> rst_view)
    {
        auto soln = ::Ewoms::data::Wells{};
    ...
    determineAquiferType(const AquiferVectors::Window<int>& iaaq)
    {
        ...
        if ((t1 == 1) && (t2 == 1)) {
            return Ewoms::data::AquiferType::CarterTracey;
        }

        if ((t1 == 0) && (t2 == 0)) {
            return Ewoms::data::AquiferType::Fetkovich;
        }
    ...
    std::shared_ptr<Ewoms::data::FetkovichData>
    extractFetkcovichData(const Ewoms::UnitSystem&               usys,
                          const AquiferVectors::Window<float>& saaq)
    {
        using M = ::Ewoms::UnitSystem::measure;

        auto data = std::make_shared<Ewoms::data::FetkovichData>();
    ...
    void restore_aquifers(const ::Ewoms::EclipseState&       es,
                          std::shared_ptr<RestartFileView> rst_view,
                          Ewoms::RestartValue&               rst_value)
    {
        using M  = ::Ewoms::UnitSystem::measure;
        ...
        for (auto aquiferID = 0*numAq; aquiferID < numAq; ++aquiferID) {
            ...
            if (aqData.type == Ewoms::data::AquiferType::Fetkovich) {
                            ...
                            std::shared_ptr<RestartFileView> rst_view)
    {
        ...
        // Group cumulatives, including FIELD.
        {
            ...
            for (const auto& gname : schedule.groupNames(sim_step)) {
                ...
                //
                // As a final wrinkle, EFlow internally stores FIELD at
