...
#include "config.h"
#endif
#include <ewoms/eclio/output/eclipsegridinspector.hh>
...
namespace Ewoms
{
EclipseGridInspector::EclipseGridInspector(Ewoms::Deck deck)
    : deck_( std::move( deck ) )
{
    init_();
}
...
void EclipseGridInspector::init_()
{
    if (!deck_.hasKeyword("COORD")) {
        EWOMS_THROW(std::runtime_error, "Needed field \"COORD\" is missing in file");
    }
    if (!deck_.hasKeyword("ZCORN")) {
        EWOMS_THROW(std::runtime_error, "Needed field \"ZCORN\" is missing in file");
    }
