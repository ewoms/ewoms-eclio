...
#ifndef EWOMS_UTILITY_SUBSTRING_H
#define EWOMS_UTILITY_SUBSTRING_H
...
namespace Ewoms {
    ...
    std::ostream& operator<<( std::ostream& stream, const Ewoms::string_view& view );

    inline std::string operator+( std::string str, const Ewoms::string_view& view ) {
        return str.append( view.begin(), view.end() );
    }

    inline std::string operator+( const Ewoms::string_view& view, const std::string& str ) {
        return view.string().append( str.begin(), str.end() );
    }

    inline bool operator==( const Ewoms::string_view& view, const std::string& rhs ) {
        return rhs.size() == view.size() &&
            std::equal( view.begin(), view.end(), std::begin( rhs ) );
    }

    inline bool operator==( const Ewoms::string_view& view, const char* rhs ) {
        return std::strlen( rhs ) == view.size() &&
               std::equal( view.begin(), view.end(), rhs );
    }

    inline bool operator==( const std::string& lhs, const Ewoms::string_view& view ) {
        return view == lhs;
    }

    inline bool operator==( const char* lhs, const Ewoms::string_view& view ) {
        return view == lhs;
    }
...
#endif //EWOMS_UTILITY_SUBSTRING_H
