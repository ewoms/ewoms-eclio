...
Ewoms::optional<std::size_t> global_index(const EclipseGrid& grid, const DeckRecord& record, std::size_t item_offset) {
...
Ewoms::optional<std::pair<std::size_t, std::size_t>> make_index_pair(const EclipseGrid& grid, const DeckRecord& record) {
...
bool is_neighbor(const EclipseGrid& grid, std::size_t g1, std::size_t g2) {
    ...
    void NNC::load_input(const EclipseGrid& grid, const Deck& deck) {
        for (const auto& keyword_ptr : deck.getKeywordList<ParserKeywords::NNC>()) {
            for (const auto& record : *keyword_ptr) {
                ...
                auto g1 = index_pair.value().first;
                auto g2 = index_pair.value().second;
    ...
    NNC NNC::serializeObject()
    {
        ...
        result.m_nnc_location = Ewoms::KeywordLocation{"NNC?", "File", 123};
        result.m_edit_location = Ewoms::KeywordLocation{"EDITNNC?", "File", 123};
        return result;
    }
