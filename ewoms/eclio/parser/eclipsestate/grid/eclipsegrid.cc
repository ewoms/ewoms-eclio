...
#include <ewoms/eclio/parser/parserkeywords/i.hh>
      ...
      m_pinchGapMode(PinchMode::ModeEnum::GAP)
{
    ...
    Ewoms::EclIO::EclFile egridfile(fileName);
      ...
      m_pinchGapMode(PinchMode::ModeEnum::GAP)
{
    ...
    if (deck.hasKeyword("MAPAXES")){
       if ((m_mapaxes.size() == 0) || ( !keywInputBeforeGdfile(deck, "MAPAXES"))) {
          const Ewoms::DeckKeyword& mapaxesKeyword = deck.getKeyword<ParserKeywords::MAPAXES>();
          m_mapaxes.resize(6);
    ...
    void EclipseGrid::initGridFromEGridFile(Ewoms::EclIO::EclFile& egridfile, std::string fileName){
        ...
        if (gridunit[0] != "METRES") {
            ...
            if (gridunit[0] == "FEET"){
                Ewoms::UnitSystem units(Ewoms::UnitSystem::UnitType::UNIT_TYPE_FIELD );
                units.to_si(length, m_coord);
                units.to_si(length, m_zcorn);
            } else if (gridunit[0] == "CM"){
                Ewoms::UnitSystem units(Ewoms::UnitSystem::UnitType::UNIT_TYPE_LAB );
                units.to_si(length, m_coord);
                units.to_si(length, m_zcorn);
            } else {
    ...
    /*
      ...
      peculiar. The situation is as follows:
        ...
        2. The ecl_grid instance will export ZCORN, COORD and ACTNUM
           ...
           If the ecl_grid is created with ZCORN as an
...
std::vector<double> EclipseGrid::createDVector(const std::array<int,3>& dims, std::size_t dim, const std::string& DKey,
    ...
    std::vector<double> EclipseGrid::activeVolume() const {
        ...
        for (std::size_t active_index = 0; active_index < this->m_active_to_global.size(); active_index++) {
            ...
            if (m_rv && m_thetav) {
                const auto& ijk = this->getIJK(global_index);
    ...
    double EclipseGrid::getCellVolume(size_t globalIndex) const {
        ...
        if (m_rv && m_thetav) {
            const auto& ijk = this->getIJK(globalIndex);
            const auto& i = std::get<0>(ijk);
            const auto& j = std::get<1>(ijk);
            //const auto& k = std::get<2>(ijk);
            auto& r = *m_rv;
    ...
    void EclipseGrid::save(const std::string& filename, bool formatted, const std::vector<Ewoms::NNCdata>& nnc, const Ewoms::UnitSystem& units) const {
        ...
        case Ewoms::UnitSystem::UnitType::UNIT_TYPE_LAB:
            ...
            break;
        default:
            EWOMS_THROW(std::runtime_error, "Unit system not supported when writing to EGRID file");
            break;
        }
        ...
        Ewoms::EclIO::EclOutput egridfile(filename, formatted);
