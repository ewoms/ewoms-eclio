...
namespace Ewoms {
    ...
    Aquancon::Aquancon(const EclipseGrid& grid, const Deck& deck)
    {
        ...
        std::vector<Ewoms::AquanconRecord> aqurecords;
    ...
    bool Aquancon::cellInsideReservoirAndActive(const Ewoms::EclipseGrid& grid, const int i, const int j, const int k)
    {
        if ( i < 0 || j < 0 || k < 0
    ...
    bool Aquancon::neighborCellInsideReservoirAndActive(const Ewoms::EclipseGrid& grid,
           const int i, const int j, const int k, const Ewoms::FaceDir::DirEnum faceDir)
    {
        switch(faceDir) {
        case FaceDir::XMinus:
