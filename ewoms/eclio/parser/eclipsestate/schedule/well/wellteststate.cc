...
namespace Ewoms {
    ...
    void WellTestState::updateForNewWTEST(const Ewoms::WellTestConfig& config)
    {
        ...
        for (auto& well: this->wells) {
            if (config.has(well.name, well.reason)) {
                ...
                if (well_config.begin_report_step != well.wtest_report_step)
                    throw std::logic_error(" Bug in eflow when using WTEST information related to well " + well.name);
                ...
                if (well.wtest_report_step != -1 || well.num_attempt != 0)
                    throw std::logic_error(" Bugs in eflow when there is WTEST request for well " + well.name);
