...
{
    using CModeVal = ::Ewoms::RestartIO::Helpers::VectorItems::IWell::Value::WellCtrlMode;
...
/*
  The fileformat used by OPM/eflow seems to work as an imperative programming
...
bool Well::handleCOMPSEGS(const DeckKeyword& keyword,
    ...
    const auto& csPair = Compsegs::processCOMPSEGS(keyword, *this->connections, *this->segments , grid,
                                                   parseContext, errors);
    auto new_connections = csPair.first;
    auto new_segments = csPair.second;
...
std::pair<std::size_t, Ewoms::optional<std::size_t>> Well::statusRange() const {
