...
VFPProdTable::VFPProdTable( const DeckKeyword& table, const UnitSystem& deck_unit_system) {
    ...
    m_flo_type = Ewoms::getFloType(header.getItem<VFPPROD::RATE_TYPE>());
    m_wfr_type = Ewoms::getWFRType(header.getItem<VFPPROD::WFR>());
    m_gfr_type = Ewoms::getGFRType(header.getItem<VFPPROD::GFR>());
    ...
    m_alq_type = Ewoms::getALQType(header.getItem<VFPPROD::ALQ_DEF>());
...
void VFPProdTable::check(const DeckKeyword& keyword, const double table_scaling_factor) {
    ...
    if (!points.empty()) {
        ...
        OpmLog::debug("VFP table for production wells has BHP versus THP not "
