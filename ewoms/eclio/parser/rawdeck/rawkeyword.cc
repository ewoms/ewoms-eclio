...
namespace {
        ...
        m_sizeType(sizeType)
    {
        ...
        if (this->m_sizeType == Raw::TABLE_COLLECTION) {
            if (size_arg == 0)
                throw std::logic_error("Bug in eflow: Attempt to create a TableCollection with zero tables. Keyword: " + name + " at " + filename + ":" + std::to_string(lineNR));
            this->m_numTables = size_arg;
        }

        if (this->m_sizeType == Raw::SLASH_TERMINATED || this->m_sizeType == Raw::UNKNOWN) {
            if (size_arg != 0)
                throw std::logic_error("Bug in eflow: Must have size_arg == 0 for SLASH_TEMINATED and UNKNOWN. Keyword: " + name + " at " + filename + ":" + std::to_string(lineNR));
        }

        if (this->m_sizeType == Raw::CODE) {
            if (size_arg != 1)
                throw std::logic_error("Bug in eflow: Must have size_arg == 1 for CODE. Keyword: " + name + " at " + filename + ":" + std::to_string(lineNR));
            this->m_fixedSize = size_arg;
        }
    }
