...
#ifndef EWOMS_SPARSEVECTOR_H
#define EWOMS_SPARSEVECTOR_H
...
namespace Ewoms
{
	...
	{
#ifndef NDEBUG
	    EWOMS_ERROR_IF(sz < 0, "The size of a SparseVector must be non-negative");
	    EWOMS_ERROR_IF(indices_.size() != data_.size(), "The number of indices of a SparseVector must equal to the number of entries");
	    int last_index = -1;
	    int num_ind = indices_.size();
	    for (int i = 0; i < num_ind; ++i) {
		int index = indices_[i];
		if (index <= last_index || index >= sz) {
		    EWOMS_THROW(std::logic_error, "Error in SparseVector construction, index is nonincreasing or out of range.");
		}
		last_index = index;
	    }
#endif
	...
	{
#ifndef NDEBUG
	    EWOMS_ERROR_IF(index < 0, "The index of a SparseVector must be non-negative (is " << index << ")");
	    EWOMS_ERROR_IF(index >= size_, "The index of a SparseVector must be smaller than the maximum value (is " << index << ", max value: " << size_ <<")");
#endif
	...
	{
#ifndef NDEBUG
	    EWOMS_ERROR_IF(nzindex < 0, "The index of a SparseVector must be non-negative (is " << nzindex << ")");
	    EWOMS_ERROR_IF(nzindex >= nonzeroSize(), "The index of a SparseVector must be smaller than the maximum value (is " << nzindex << ", max value: " << nonzeroSize() <<")");
#endif
