...
void ERsm::load_block(std::deque<std::string>& lines, std::size_t& vector_length) {
    ...
    while (true) {
        ...
        if (vector_length == 0) {
            if (Ewoms::holds_alternative<std::vector<double>>(this->time)) {
                double d = std::stod(data_row[0]) * mult_list[0];
                Ewoms::get<std::vector<double>>( this->time ).push_back( d );
            } else {
                TimeStampUTC ts = make_timestamp(data_row[0]);
                Ewoms::get<std::vector<TimeStampUTC>>( this->time ).push_back( ts );
            }
        }
        block_size += 1;
    }
...
const std::vector<TimeStampUTC>& ERsm::dates() const {
    ...
    return Ewoms::get<std::vector<TimeStampUTC>>( this->time );
}
...
const std::vector<double>& ERsm::days() const {
    ...
    return Ewoms::get<std::vector<double>>( this->time );
}
...
bool ERsm::has_dates() const {
    return Ewoms::holds_alternative<std::vector<TimeStampUTC>>(this->time);
}
