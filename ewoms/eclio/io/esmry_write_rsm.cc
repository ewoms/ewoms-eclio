...
namespace {
    ...
    void write_header_columns(std::ostream& os, const std::string& time_column, const std::vector<Ewoms::EclIO::SummaryNode>& vectors, std::function<void(std::ostream&, const Ewoms::EclIO::SummaryNode&)> print_element, char prefix = ' ') {
        os << prefix;
...
void ESmry::write_rsm_file(Ewoms::optional<Ewoms::filesystem::path> filename) const {
