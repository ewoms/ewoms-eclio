    ...
    summaryNodes { }
{
    ...
    if ((!loadBaseRunData) && (Ewoms::filesystem::exists(lodFileName)))
        lodEnabeled = true;
    else
        lodEnabeled = false;
    ...
    if (lodEnabeled)
    {
        ...
        while (specInd >= 0) {
            ...
            Ewoms::filesystem::path smspecFile(std::get<0>(smryArray[specInd]));
...
void ESmry::inspect_lodsmry()
{
    std::string arrName;
    int64_t arr_size;
    Ewoms::EclIO::eclArrType arrType;
    int sizeOfElement;
    ...
    if (formattedFiles[0]) {
        ...
        Ewoms::EclIO::readFormattedHeader(fileH, arrName, arr_size, arrType, sizeOfElement);
    } else {
        fileH.open(lodFileName, std::ios::in |  std::ios::binary);
        Ewoms::EclIO::readBinaryHeader(fileH, arrName, arr_size, arrType, sizeOfElement);
    }

    if ((arrName != "KEYCHECK") or (arrType != Ewoms::EclIO::CHAR))
        EWOMS_THROW(std::invalid_argument, "reading keycheck, invalid lod file");

    std::vector<std::string> keycheck;

    if (formattedFiles[0]) {
        uint64_t size = Ewoms::EclIO::sizeOnDiskFormatted(arr_size, Ewoms::EclIO::CHAR, sizeOfChar) + 1;
        std::string fileStr = read_string_from_disk(fileH, size);

        keycheck = Ewoms::EclIO::readFormattedCharArray(fileStr, arr_size, 0, sizeOfChar);
    } else {
        keycheck = Ewoms::EclIO::readBinaryCharArray(fileH, arr_size);
    }
    ...
    for (size_t n = 0; n < nVect; n++) {
        ...
        if (keyword[n].size() > 24) {
            if (keyword[n].substr(0,24) != test_str) {
                EWOMS_THROW(std::invalid_argument, "keycheck not maching keyword array");
            }
        } else {
    ...
    if ((arrName != "RSTEP   ") or (arrType != Ewoms::EclIO::LOGI))
        EWOMS_THROW(std::invalid_argument, "reading rstep, invalid lod file");

    std::vector<bool> rstep;

    if (formattedFiles[0]) {
        uint64_t size = Ewoms::EclIO::sizeOnDiskFormatted(arr_size, Ewoms::EclIO::LOGI, sizeOfLogi) + 1;
        std::string fileStr = read_string_from_disk(fileH, size);

        rstep = Ewoms::EclIO::readFormattedLogiArray(fileStr, arr_size, 0);
    } else {
    ...
    if (formattedFiles[0])
        lod_arr_size = sizeOnDiskFormatted(nTstep, Ewoms::EclIO::REAL, sizeOfReal);
    else
        lod_arr_size = sizeOnDiskBinary(nTstep, Ewoms::EclIO::REAL, sizeOfReal);
...
void ESmry::Load_from_lodsmry(const std::vector<int>& keywIndVect) const
{
    ...
    for (auto ind : keywIndVect) {
        ...
        arrName = Ewoms::EclIO::trimr(arrName);

        std::string checkName = "V" + std::to_string(ind);

        if (arrName != checkName)
            EWOMS_THROW(std::invalid_argument, "lodsmry, wrong header expecting  " + checkName + " found " +  arrName);

        if (formattedFiles[0]) {
            uint64_t size_buffer = lod_arr_size + 1;
            std::string fileStr = read_string_from_disk(fileH, size_buffer);
            vectorData[ind] = Ewoms::EclIO::readFormattedRealArray(fileStr, nTstep, 0);
        } else {
...
void ESmry::LoadData(const std::vector<std::string>& vectList) const
{
    ...
    for (auto key : vectList){
        if (!hasKey(key))
            EWOMS_THROW(std::invalid_argument, "error loading key " + key );
            ...
            for (auto ind : keywIndVect) {
                ...
                if (it == arrayPos[specInd].end()) {
                    ...
                    if (formattedFiles[specInd]) {
                        ...
                        vectorData[ind].push_back(Ewoms::EclIO::flipEndianFloat(value));
                    }
                }
            }
        }
    ...
    if (lodEnabeled) {
        ...
        for (auto ministep : timeStepList) {
            ...
            if (formattedFiles[specInd]) {
                ...
                char* buffer;
                size_t size = sizeOnDiskFormatted(nParamsSpecFile[specInd], Ewoms::EclIO::REAL, sizeOfReal) + 1;
                buffer = new char [size];
                fileH.read (buffer, size);
                ...
                while (rest > 0) {
                    ...
                    for (int i = 0; i < num; i++) {
                        ...
                        if ((keywpos[p] > -1) && (!vectorLoaded[keywpos[p]]))
                            vectorData[keywpos[p]].push_back(Ewoms::EclIO::flipEndianFloat(value));
...
ESmry::getListOfArrays(std::string filename, bool formatted)
{
    ...
    while (!endOfFile)
    {
        ...
        if (formatted)
        {
            ...
            num = static_cast<int64_t>(Ewoms::EclIO::flipEndianInt(num_int));
        ...
        t1 = std::make_tuple(Ewoms::EclIO::trimr(arrName), filePos);
...
bool ESmry::make_lodsmry_file()
{
    ...
    if (!fromSingleRun)
        EWOMS_THROW(std::invalid_argument, "creating lodsmry file only possible when loadBaseRunData=false");

    Ewoms::filesystem::path path = inputFileName.parent_path();
    Ewoms::filesystem::path rootName = inputFileName.stem();
    Ewoms::filesystem::path smryDataFile;
    ...
    if (Ewoms::EclIO::fileExists(smryDataFile))
    {
        return false;
        ...
        {
            Ewoms::EclIO::EclOutput outFile(smryDataFile, formattedFiles[0], std::ios::out);
