...
std::vector<NNCentry> EGrid::get_nnc_ijk()
{
    ...
    for (size_t n=0; n< nnc1_array.size(); n++){
        ...
        if (transnnc_array.size() > 0)
            res_vect.push_back(NNCentry{ijk1[0], ijk1[1], ijk1[2], ijk2[0], ijk2[1], ijk2[2], transnnc_array[n]});
        else
            res_vect.push_back(NNCentry{ijk1[0], ijk1[1], ijk1[2], ijk2[0], ijk2[1], ijk2[2], -1.0 });
    }
...
void EGrid::load_nnc_data()
{
    if ((nnc1_array_index > -1) && (nnc2_array_index > -1)) {
        ...
        nnc1_array = getImpl(nnc1_array_index, Ewoms::EclIO::INTE, inte_array, "inte");
        nnc2_array = getImpl(nnc2_array_index, Ewoms::EclIO::INTE, inte_array, "inte");

        if ((Ewoms::filesystem::exists(initFileName)) && (nnc1_array.size() > 0)){
            Ewoms::EclIO::EInit init(initFileName);
...
int EGrid::global_index(int i, int j, int k) const
{
    if (i < 0 || i >= nijk[0] || j < 0 || j >= nijk[1] || k < 0 || k >= nijk[2]) {
        EWOMS_THROW(std::invalid_argument, "i, j or/and k out of range");
    }
...
std::array<int, 3> EGrid::ijk_from_active_index(int actInd) const
{
    if (actInd < 0 || actInd >= nactive) {
        EWOMS_THROW(std::invalid_argument, "active index out of range");
    }
...
std::array<int, 3> EGrid::ijk_from_global_index(int globInd) const
{
    if (globInd < 0 || globInd >= nijk[0] * nijk[1] * nijk[2]) {
        EWOMS_THROW(std::invalid_argument, "global index out of range");
    }
