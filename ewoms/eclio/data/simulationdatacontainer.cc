...
namespace Ewoms {
    ...
    void SimulationDataContainer::setCellDataComponent( const std::string& key ,
        ...
        if (component >= m_num_phases)
            EWOMS_THROW(std::invalid_argument, "The component number: " << component << " is invalid");

        if (cells.size() != values.size())
            EWOMS_THROW(std::invalid_argument, "size mismatch between cells and values");
        ...
        if (data.size() != m_num_phases * m_num_cells)
            EWOMS_THROW(std::invalid_argument , "Can currently only be used on fields with num_components == num_phases (i.e. saturation...) ");
        ...
        for (size_t i = 0; i < cells.size(); i++) {
            if (size_t(cells[i]) < m_num_cells) {
                ...
                EWOMS_THROW(std::invalid_argument , "The cell number: " << cells[i] << " is invalid.");
            }
        }
    }
